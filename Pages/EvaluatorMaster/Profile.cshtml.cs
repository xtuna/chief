...
namespace chief.Pages.EvaluatorMaster
{
    public class ProfileModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetAsync(int? id)
        {
            ...
            // Calculate the counts of different request types based on EvaluatorId (change to the correct property name)
            PendingRequests = await _context.Applications.CountAsync(a => a.Id == id && a.Status == "Pending");
            EvaluatedRequests = await _context.Applications.CountAsync(a => a.Id == id && a.Status == "Evaluated");
            MissedRequests = await _context.Applications.CountAsync(a => a.Id == id && a.Status == "Missed");
